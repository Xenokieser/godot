...
#include "webview/shared/browser/extension_util.h"
...
void ClientHandler::OnLoadError(CefRefPtr<CefBrowser> browser,
  ...
  if (errorCode == ERR_cef_ABORTED)
    return;

  // Don't display an error for external protocols that we allow the OS to
  // handle. See OnProtocolExecution().
  if (errorCode == ERR_cef_UNKNOWN_URL_SCHEME) {
...
bool ClientHandler::OnCertificateError(CefRefPtr<CefBrowser> browser,
  ...
  if (cert_error == ERR_cef_CERT_AUTHORITY_INVALID &&
