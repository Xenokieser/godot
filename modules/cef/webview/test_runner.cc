...
#include "webview/test/binding_test.h"
#include "webview/client_handler.h"
#include "webview/test/dialog_test.h"
#include "webview/main_context.h"
#include "webview/test/media_router_test.h"
#include "webview/test/preferences_test.h"
#include "webview/resource.h"
#include "webview/test/response_filter_test.h"
#include "webview/root_window_manager.h"
#include "webview/test/scheme_test.h"
#include "webview/test/server_test.h"
#include "webview/test/urlrequest_test.h"
#include "webview/test/window_test.h"
#include "webview/shared/browser/resource_util.h"
...
#define CASE(code) \
  ...
  switch (code) {
    CASE(ERR_NONE);
    CASE(ERR_cef_FAILED);
    CASE(ERR_cef_ABORTED);
    CASE(ERR_cef_INVALID_ARGUMENT);
    CASE(ERR_cef_INVALID_HANDLE);
    CASE(ERR_cef_FILE_NOT_FOUND);
    CASE(ERR_cef_TIMED_OUT);
    CASE(ERR_cef_FILE_TOO_BIG);
    CASE(ERR_cef_UNEXPECTED);
    CASE(ERR_cef_ACCESS_DENIED);
    CASE(ERR_cef_NOT_IMPLEMENTED);
    CASE(ERR_cef_CONNECTION_CLOSED);
    CASE(ERR_cef_CONNECTION_RESET);
    CASE(ERR_cef_CONNECTION_REFUSED);
    CASE(ERR_cef_CONNECTION_ABORTED);
    CASE(ERR_cef_CONNECTION_FAILED);
    CASE(ERR_cef_NAME_NOT_RESOLVED);
    CASE(ERR_cef_INTERNET_DISCONNECTED);
    CASE(ERR_cef_SSL_PROTOCOL_ERROR);
    CASE(ERR_cef_ADDRESS_INVALID);
    CASE(ERR_cef_ADDRESS_UNREACHABLE);
    CASE(ERR_cef_SSL_CLIENT_AUTH_CERT_NEEDED);
    CASE(ERR_cef_TUNNEL_CONNECTION_FAILED);
    CASE(ERR_cef_NO_SSL_VERSIONS_ENABLED);
    CASE(ERR_cef_SSL_VERSION_OR_CIPHER_MISMATCH);
    CASE(ERR_cef_SSL_RENEGOTIATION_REQUESTED);
    CASE(ERR_cef_CERT_COMMON_NAME_INVALID);
    CASE(ERR_cef_CERT_DATE_INVALID);
    CASE(ERR_cef_CERT_AUTHORITY_INVALID);
    CASE(ERR_cef_CERT_CONTAINS_ERRORS);
    CASE(ERR_cef_CERT_NO_REVOCATION_MECHANISM);
    CASE(ERR_cef_CERT_UNABLE_TO_CHECK_REVOCATION);
    CASE(ERR_cef_CERT_REVOKED);
    CASE(ERR_cef_CERT_INVALID);
    CASE(ERR_cef_CERT_END);
    CASE(ERR_cef_INVALID_URL);
    CASE(ERR_cef_DISALLOWED_URL_SCHEME);
    CASE(ERR_cef_UNKNOWN_URL_SCHEME);
    CASE(ERR_cef_TOO_MANY_REDIRECTS);
    CASE(ERR_cef_UNSAFE_REDIRECT);
    CASE(ERR_cef_UNSAFE_PORT);
    CASE(ERR_cef_INVALID_RESPONSE);
    CASE(ERR_cef_INVALID_CHUNKED_ENCODING);
    CASE(ERR_cef_METHOD_NOT_SUPPORTED);
    CASE(ERR_cef_UNEXPECTED_PROXY_AUTH);
    CASE(ERR_cef_EMPTY_RESPONSE);
    CASE(ERR_cef_RESPONSE_HEADERS_TOO_BIG);
    CASE(ERR_cef_CACHE_MISS);
    CASE(ERR_cef_INSECURE_RESPONSE);
